...
class DeepContourNet(object):
    ...
    def build_net(self, input, scope=""):
        ...
        with tf.name_scope(scope, "dcnn"):
            ...
            # down sample
            conv0 = conv2d(input, 64, [3, 3], scope="conv0", padding="SAME")
            conv0_out = conv2d(conv0, 64, [3, 3], scope="conv0_out", padding="SAME")
            maxpool0 = tf.nn.max_pool(conv0_out, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="SAME", name="maxpool0")

            conv1 = conv2d(maxpool0, 128, [3, 3], scope="conv1", padding="SAME")
            conv1_out = conv2d(conv1, 128, [3, 3], scope="conv1_out", padding="SAME")
            maxpool1 = tf.nn.max_pool(conv1_out, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="SAME", name="maxpool1")

            conv2 = conv2d(maxpool1, 256, [3, 3], scope="conv2", padding="SAME")
            conv2_out = conv2d(conv2, 256, [3, 3], scope="conv2_out", padding="SAME")
            ...
            deconv0_out = conv2d(deconv0_concat, 128, [3, 3], scope="deconv0_out", padding="SAME")
            ...
            deconv1_out = conv2d(deconv1_concat, 64, [3, 3], scope="deconv1_out", padding="SAME")
            ...
            deconv_contour0_out = conv2d(deconv_contour0_concat, 128, [3, 3], scope="deconv_contour0_out", padding="SAME")
            ...
            deconv_contour1_out = conv2d(deconv_contour1_concat, 128, [3, 3], scope="deconv_contour1_out", padding="SAME")
        ...
        with tf.Session() as sess:
            ...
            output_mask, output_contour = sess.run([self.output_mask, self.output_contour],
                                                   feed_dict={self.x:x_test,
                                                              self.output_mask:mask_dummy,
                                                              self.output_contour:contour_dummy})
